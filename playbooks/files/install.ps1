...
# Determine flavor
if ($FlavorParam -and $palette.Contains($FlavorParam)) {
    $flavor = $FlavorParam
}
else {
    $flavorIndex = $Host.UI.PromptForChoice("Flavor", "Which flavor do you want to use?", $flavorChoices, 3)
    $flavor = $palette.Keys[$flavorIndex]
}
$colors = $palette[$flavor]

# Determine flat appearance
if ($PSBoundParameters.ContainsKey('FlatAppearanceParam')) {
    $flatAppearance = $FlatAppearanceParam
}
else {
    $flatAppearanceIndex = $Host.UI.PromptForChoice("Appearance", "Do you want to use the flat appearance?", ('&Yes', '&No'), 1)
    $flatAppearance = $flatAppearanceIndex -eq 0
}
